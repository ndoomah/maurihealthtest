...
def postLoop(postDiv, array, driver, searchterm):
    ...
    # scraping the post's title/name
        ...
        contents = post.find_elements_by_tag_name('p')
        c = ""
        ...
        if dis == "":
            ...
            try:
                ...
                l = post.find_element_by_xpath("//div[@class='_lid fsm fwn fcg']/a[@class='_62xw']")
                location = l.text
                '''except:
                    l = post.find_element_by_xpath("//span[@class='fsm fwn fcg']/a[@class='_5pcq'][2]")
                    location = l.text'''
            except:
...
class Loop:
    ...
    def __init__(self,postDiv, array, driver, searchterm):
    ...
    def postLoop(self):
        ...
        # scraping the post's title/name
            ...
            # scraping the post's date
            date = self.driver.find_element_by_class_name("timestampContent")
            #date = driver.find_element_by_xpath("//span[@class='timestampContent']")
            d = date.text

            # scraping the post's contents
            contents = self.driver.find_elements_by_tag_name('p')
            c = ""
            for content in contents:
                c = c + content.text + "\t"
            ...
            if dis == "":
                ...
                #if location is already enabled on the post
                try:
                    l = self.driver.find_element_by_xpath("//a[contains (@id, 'js_')]")
                    location = l.text
                except:
                    #else extract location details from the content
            ...
            time.sleep(1)
            el = self.driver.find_element_by_class_name('_n9')
...
def scrape_page_test(url, driver):
    data_arr = []
    ...
    postLoop(firstdivloop,data_arr,driver,disease)
    ...
    try:
        ...
        postLoop(secdivloop,data_arr,driver, disease)
        ...
        for scrollContainer in scrollContainers:
            ...
            postLoop(scroll_div, data_arr, driver, disease)
            time.sleep(1)
    except:
